...
subcarrier_amplitude_list = np.zeros((400, 3, 3, 30),dtype=object).tolist()
subcarrier_phase_list = [0 for i in range(400)]  # 描述相位载波初始化
subcarrier_phase_list_filter = []  # 描述振幅过滤载波初始化
subcarrier_amplitude_list_filter = []  # 描述相位过滤载波初始化
distinguish_list=np.zeros((400, 3, 3, 30)) #这个用来存放全部原始数据n*3*3*30格式，用于识别
